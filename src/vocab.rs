...
pub const BASE_RE: &str = r#"(?:^.$)|(?:^ ?(?:[a-z]+):\/\/(?:(?:[a-z0-9]+\.?))+(?::[0-9]{1,5})?\/?$)|(?:^ ?(?:(?:[\[{("']|__)[A-Za-z0-9]+(?:[\]})"']|__))$)|(?:^(?:[0-9]{1,4}(?:\.[0-9]{1,3})?)$)|(?:^(?:[\/\\_\-\.]|(?:__)|(?:::))[A-Za-z0-9_]+$)|(?:^(?:\/[a-zA-Z0-9]+)+$)|(?:^ ?[@&!?#$\*\(\[][A-Za-z0-9]+$)|(?:^ ?[A-Za-z0-9_]+(?:\(|%|(?:::)|(?:->)|(?:\.))$)|(?:^(?: ?(?:(?:[0-9]{1,4}(?:[A-Za-z]+)?)|(?:[A-Za-z]+(?:[0-9]{1,4})?)|[A-Za-z]+))$)|(?:^ ?(?:(?:[0-9]{1,4}(?:[a-zA-Z]+)?)|(?:[a-zA-Z]+(?:[0-9]{1,4})?)|[A-Z]+)(?:(?:[_\-.]|::|->|/)(?:(?:[0-9]{1,4}(?:[a-zA-Z]+)?)|(?:[a-zA-Z]+(?:[0-9]{1,4})?)|[A-Z]+))*$)|(?:^[\u3400-\u4DBF\u4E00-\u9FFF]+$)|(?:^(?:(?:(?:[[:punct:] DCU]+)?[[:space:]DCU]*)|(?:[[:space:]DCU]*(?:[[:punct:] DCU]+)?))$)|(?:^(?:<\/?[A-Za-z]+(?:>|(?:\/>)|(?: \/>)|(?: @?[A-Za-z]+(?:=")?))?)$)|(?:^ ?(?:(?:[A-Za-z]+(?:'[A-Za-z]+)?)|(?:[0-9]{1,4}))(?:(?: [A-Za-z]+(?:'[A-Za-z]+)?)|(?: [0-9]{1,4})){0,2}$)|(?:^ ?(?:(?:!=)|(?:!==)|(?:==)|(?:=)|(?::=)|(?::)) ?[A-Za-z0-9_]+(?:\.[0-9]{1,3})?$)|(?:^(?:, ?[A-Za-z0-9]+)$)|(?:^ (?:[A-Za-z]+(?:=(?:"|'))?)$)|(?:^[A-Za-z\-]+: $)"#;
