...
pub const ENV_VARIABLE: &str = "TOKENGEEX_PARALLELISM";

// Reading/Writing this variable should always happen on the main thread
static mut USED_PARALLELISM: bool = false;

/// Check if the TOKENGEEX_PARALLELISM env variable has been explicitly set
#[allow(dead_code)]
...
/// Get the currently set value for `TOKENGEEX_PARALLELISM` env variable
...
/// Set the value for `TOKENGEEX_PARALLELISM` for the current process
#[allow(dead_code)]
...
///
/// The choice is made according to the currently set `TOKENGEEX_PARALLELISM` environment variable.
    ...
    S: Iterator<Item = P::Item>,
{
    ...
    /// based solely on the `TOKENGEEX_PARALLELISM` environment variable
    fn into_maybe_par_iter(self) -> CondIterator<P, S>;
    /// Convert ourself in a CondIterator, that will be executed either in parallel or serially,
    /// based on both the `TOKENGEEX_PARALLELISM` environment variable and the provided bool.
